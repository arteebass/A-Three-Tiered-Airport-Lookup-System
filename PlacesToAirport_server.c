...
//function to copy information in one node to another
...
//logic is borrowed from github
void kNearest(kd_node_t *head, kd_node_t list[], double coords[], int dim){
	if(head!=NULL){
		if(head->x[dim]>coords[dim]){//if head dist is greater than user dist
			kNearest(head->left, list, coords, (dim+1)%2);//recursively go left
			if(list[4].x[0] == 0 && list[4].x[1] == 0){
				//similar to reassign, but need to take state and distance
		...
		else{//same as above just for going right
			kNearest(head->right, list, coords, (dim+1)%2);//recursively go right
			if(list[4].x[0] == 0 && list[4].x[1] == 0){
				//same reassign as above
				strcpy(list[4].city, head->city);
				strcpy(list[4].code, head->code);
				strcpy(list[4].state, head->state);
				list[4].x[0] = head->x[0];
...
returnTypeA * ptoa_1_svc(LatLon *argp, struct svc_req *rqstp)
{
    static returnTypeA  result;
	//free memory before each call
	...
	kd_node_t kdNode[parseList.size() - 1]; //size is equal to number of lines we get from file
	//creating a iterator 
	kd_node_t* hdNode = &kdNode[0];
	//create an iterator for the head node
	...
	for(int i=1; i < parseList.size(); i++){
		...
		//lots of conversions to get them to work with char[]
		string CODE;
		...
		int NAMESIZE;
		NAMESIZE = (parseList[i].substr(20,40).size()) - 1; 
		...
		//setting all of the data to the iterator and then moving to the next node
		iter->x[0] = aaaalat;
		iter->x[1] = aaaalon;
		...
		//copying data into a new node to put into our vector
	...
	kd_node_t list[5];
	//initialize for the knearest function
	for(int i = 0; i<5; i++){
		list[i].x[0]=0;
		list[i].x[1]=0;
		list[i].x[2]=10000;
	...
	//copying info into our linked list to be sent back to ClientToPlaces_server
	for(int i = 0; i < 6; i++){
